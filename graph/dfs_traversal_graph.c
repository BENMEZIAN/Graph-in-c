...
void internalDFS(Graph* graph, int vertex); // Recursive DFS
void DFS(Graph* graph, int vertex); // Recursive DFS
void DfsIterative(Graph* graph, int startVertex); // Iterative DFS
...
void internalDFS(Graph* graph, int vertex){
    ...
    while(node != NULL){
        int connectedVertex = node->vertex;
        if(graph->visited[connectedVertex] == 0){
            internalDFS(graph, connectedVertex);
        }
        node = node->next;
    }
}
...
void DFS(Graph* graph, int vertex){
    // Reset all vertices as not visited before starting DFS from a new vertex
    for(int i = 0; i < graph->V; ++i){
        graph->visited[i] = 0;
    }
    
    // Call the actual DFS function
    internalDFS(graph, vertex);
}
