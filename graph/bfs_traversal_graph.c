...
int main() {
    ...
    addEdge(graph, 3, 4, 4, false);
    
    printf("BFS (start vertex 0) : ");
    BFS(graph,0);
...
void BFS(Graph* graph,int startVertex){
	...
	init(&q);
    graph->visited[startVertex] = 1; // set 1 for the start vertex (visited)
    enqueue(&q, startVertex); // make in the queue
    
    while(!isEmpty(&q)){
    	int currentVertex = dequeue(&q); // dequeue the front of the queue and assign it to current vertex
        printf("%d ", currentVertex); // print the current vertex
        
		AdjNode *temp = graph->array[currentVertex].head; // point at the head of the adjacency list of this current vertex
		
		while(temp != NULL){
            int adjVertex = temp->vertex; // iterate through the list
            if (graph->visited[adjVertex] == 0){ // if the adjvertex is not visited
                graph->visited[adjVertex] = 1; // set  1 for this vertex (visited)
                enqueue(&q, adjVertex); // make in the queue
            } 
            temp = temp->next; // next adjacent node
        }        
	}
	
	// reset all vertexes as not visited
	for(int i = 0; i < graph->V; ++i){
        graph->visited[i] = 0;
    }
}
