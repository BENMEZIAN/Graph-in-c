...
void floydWarshall(Graph* graph){
    int V = graph->V;
    int dist[V][V];
    int next[V][V];  // To store the next vertex in the shortest path

    // Initialize the distance matrix and next array
    for (int i = 0; i < V; i++) {
        for (int j = 0; j < V; j++) {
            if (i == j) {
                dist[i][j] = 0;
    ...
    // Populate the distance matrix with edge weights and set next accordingly
    for (int i = 0; i < V; i++) {
        AdjNode* temp = graph->array[i].head;
        while (temp != NULL) {
            dist[i][temp->vertex] = temp->weight;
            next[i][temp->vertex] = temp->vertex;
            temp = temp->next;
        }
    }
    ...
    for (int k = 0; k < V; k++) {
        for (int i = 0; i < V; i++) {
            for (int j = 0; j < V; j++) {
                if (dist[i][k] != MIN && dist[k][j] != MIN && dist[i][k] + dist[k][j] < dist[i][j]) {
                    dist[i][j] = dist[i][k] + dist[k][j];
                    next[i][j] = next[i][k];  // Update next for shortest path
                }
            }
        }
    }
    ...
    // Print column headers (target vertices)
    printf("%7s", "");
    for (int j = 0; j < V; j++) {
        printf("%7d", j);
    }
    ...
    for (int i = 0; i < V; i++) {
        // Print row header (source vertex)
        printf("%7d", i);
