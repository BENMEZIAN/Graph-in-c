...
int main() {
    ...
    addEdge(graph, 4, 5, 6, false); // Adding more edges
    addEdge(graph, 5, 6, 7, false); // Adding more edges
    addEdge(graph, 6, 0, 8, false); // Adding more edges
    addEdge(graph, 3, 5, 9, false); // Adding more edges
...
void kruskalMST(Graph* graph) {
    ...
    int V = graph->V;
    Edge* edges = (Edge*)malloc(V * (V - 1) / 2 * sizeof(Edge));
    int edgeCount = 0;
    ...
    for (int i = 0; i < V; ++i) {
        ...
        while (current != NULL) {
            if (i < current->vertex) { // To avoid adding duplicates
                edges[edgeCount].src = i;
                edges[edgeCount].dest = current->vertex;
    ...
    // Create a disjoint set for cycle detection
    DisjointSets* disjointSet = createDisjointSets(V);

	printf("Minimum Spanning Tree (Kruskal's Algorithm):\n");
    
    int totalWeight = 0; // Variable to store the sum of edge weights

    for (int i = 0; i < edgeCount; ++i) {
        int src = edges[i].src;
        int dest = edges[i].dest;

        if (find(disjointSet, src) != find(disjointSet, dest)) {
            printf("%d - %d (%d)\n", src, dest, edges[i].weight);
            unionSets(disjointSet, src, dest);
            totalWeight += edges[i].weight; // Add edge weight to the total
        }
    }
    ...
    // Clean up
    destroyDisjointSets(disjointSet);
    free(edges);
}
