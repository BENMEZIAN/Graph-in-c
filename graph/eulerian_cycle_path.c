...
int main() {
    ...
    int res = isEulerian(graph); // res stores odd degree vertices
    
	switch(res){
		case 0: printf("The graph is eulerian (has eulerian cycle)");
		break;
		
		case 1: printf("The graph is semi-eulerian (has eulerian path)");
		break;
		
		default: printf("The graph is not Eulerian");
		break;
	}
...
bool isConnected(Graph* graph) {
    ...
    for (int i = 0; i < graph->V; ++i) {
        if (graph->array[i].head != NULL && graph->visited[i] == 0) {
